...
<body>
	<div id="layout1">
		<div id="centerCon" position="center" style="">
        	<div class="box2" panelTitle="可在下方添加新的为未被关联的检查项" showStatus="false">
			...
			<input type="hidden" id="initEndTime" value="${initEndTime }" />
			  ...
			  <table class="detail-list" >
					...
					<td  style="width:35%;text-align:right">编号：</td>
					<td  style="width:35%;text-align:left"><input type="text" id="code" class="input119" name="code" value="${title }"/></td>
				<%-- 	
					 <td  style="width:15%;text-align:right">名称：</td>
					 <td  style="width:25%;text-align:left"><input type="text" id="name" class="input119" name="name" value="${content }"/></td>
				 --%>
...
<script type="text/javascript">
	...
	function initGrid(params){
		...
		grid = $("#dataBasic").quiGrid({
			columns:[	
					    { display: '指标项编号', name: 'check_item_code', align: 'left', width: "33%", showTitle:true,
					    },
					    ...
					    { display: '操作',	isAllowHide: false, align: 'left', width:80,
			                 ...
			                 render:function(rowdata,rowindex,value,column){
			                	 var html='<div class="padding_top4 padding_left5">'
			                		...
			                		// html += '<span   class="img_list"  title="添加" onclick="onDelete(\'' + rowdata.check_item_id+'\')"></span>'
			                		html+=  '<a href="javascript:;"><span class="icon_add" onclick="onDelete(\'' + rowdata.check_item_id+'\')">新增</span></a>'
			                		 return html;
		   ...
		   // url: '${path}/eqAsCheck/getList?sbId=4028802d5e80440d015e8045d22f0000',
				   url: '${path}/eqAsCheck/getUnCheckList?sbId='+sbidsbid ,
		  ...
		  /*   toolbar:{
		           	 items:[
		           	      //  {text: '勾选添加', click: addUnit, iconClass: 'icon_add'},
...
function handleResult(result){
	...
	function addUnit() {
		...
		Dialog.confirm("确定要添加吗？",function(){
			$.post('${path }/eqAsCheck/addUnit?sbId='+sbidsbid,
					//获取所有选中行
					getSelectIds(grid),
					function(result){
