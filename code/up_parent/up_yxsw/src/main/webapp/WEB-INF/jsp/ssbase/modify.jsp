		                 ...
		                 <table class="detail-list" id="ssInfoDetailTable">
		                 	<tr>
		                 		...
		                 		<td>
									...
									<input type="hidden" name="parentId" id="parentId" value="${ss.parentId }" />
									<input type="hidden" name="ssId" value="${ss.ssId }" />
		                 		...
		                 		<td>
									<input type="text" name="name" id="name" class="validate[required,length[0,42]]" value="${ss.name }" style="width:180px;"/>
		                 		</td>
		                 	</tr>
		                 	<tr>
		                 		<th><span><em style="color:red;">*</em></span>设施编号：</th>
		                 		<td><input type="text" name="code" id="code" class="validate[required,custom[notChinese],length[0,10],ajax[${path }/ssbase/validateSSCode?old='${ss.code }' | * 设施编号已存在]]" value="${ss.code }" style="width:180px;"/></td>
		                 		<th>是否扫描二维码：</th>
		                 		<td><select labelField="value" valueField="key" selWidth="180" name="byzd" id="byzd" selectedValue="${ss.byzd }" url="${path}/platform/getSupportData" params='{"parentNodeId":"${DELFLAG_DICT }"}' ></select></td>
		                 	</tr>
		                 	<tr>
		                 		<th>功能说明：</th>
		                 		<td colspan="3">
									<textarea maxNum="682" id="_function" name="_function" style="width:50%;height:110px;"/>${ss.function }</textarea>
								</td>
		                 	</tr>
		                 	<tr>
								<th>备注：</th>
								<td colspan="3">
									<textarea maxNum="682" name="remark"  style="width:50%;height:110px;"/>${ss.remark }</textarea>
								</td>
							</tr>
		                 </table>
	            	</div>
		...
		<div class="list-btm">
	    	<div class="list-btm-div">
	            <div class="div-btn-sbt">
	            	<button class="btm-button" type="button"  onclick="doModify();"><span class="ok-icon">保存</span></button>
	            </div>
	        </div>
	    </div>
     </div>
...
<script type="text/javascript">
	...
	function selectParent(){
		Dialog.open({ID:"c1", InnerHtml: "", Title:"选择父类设施", Width:340, Height:400, });
	...
	function zTreeBeforeClick(treeId, treeNode, clickFlag) {
		...
		var nodes = treeObj.getNodesByParam("id", "${ss.ssId}", null);
		...
		if(id == "${ss.ssId}"){
			...
			nodeIds = check(nodes[0],childIds);
			var nodeIdsStr = nodeIds.join(",");
			if(nodeIdsStr.indexOf(id)>-1){
				flag = false;
			}
		}
		if(flag == false){
	...
	function check(node,childIds){
		if (node.isParent) {
	        var childrenNodes = node.children;
	        if (childrenNodes) {
	            for (var i = 0; i < childrenNodes.length; i++) {
	            	childIds.push(childrenNodes[i].id);
	            	childIds = check(childrenNodes[i], childIds);
	            }
	        }
	    }
		return childIds;
	...
	function doModify(){
		...
		if(valid){
			$.ajax({
	            cache: true,
	            type: "POST",
	            url:'${path}/ssbase/doModify',
	            data:$('#myFormId').serialize(),
	            error: function(err) {
	            	parent.Dialog.alert("提交保存失败");
	            },
	            success: function(responseText){
	            	
	            	parent.Dialog.alert(responseText.message, function(){
	            		if(responseText.flag){
	            			parent.SSBaseInfo.refreshTree();
		                    parent.jQuery("#ss-detail-iframe").attr("src","${path}/ssbase/showDetail?ssId=${ss.ssId}&parentName=" + $("#parentName").val() );
	            		}else{
	            			 Dialog.close();
	            		}
	                });
