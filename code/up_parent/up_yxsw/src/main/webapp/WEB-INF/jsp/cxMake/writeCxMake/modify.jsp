...
<style type="text/css">
	...
	.num_input{width: 99%;height: 100%;border: none;text-align: center; }
	.text_input{width: 98%;max-height: 100px; resize: none;border:none;text-align: center;}
							...
							<tr makeTmpItemId="${makeTmpItem.makeTmpItemId }" inputType="${makeTmpItem.tblx }" class="makeTmpItemTr">
							    ...
							    <c:if test="${makeTmpItem.tblx eq TEXT_INPUT_TYPE }">
							    	...
							    	<td class="padding0" title="${makeTmpItem.jlsbz }">
							    		<div class="text_input_div">
							    			<textarea class="text_input <c:if test="${!(makeTmp.jlsbz eq JLSBZ)}">validate[length[0,1000]]</c:if>" <c:if test="${ makeTmp.jlsbz eq JLSBZ}">disabled="disabled" style="background-color:#eeeeee!important"</c:if>>${makeTmpItem.jlsbz }</textarea>
							    		</div>
							    	</td>
							    ...
							    <c:if test="${makeTmpItem.tblx eq NUM_INPUT_TYPE }">
							    	...
							    	<td class="padding0">
							    		<input class="num_input <c:if test="${!(makeTmp.jlsbz eq JLSBZ)}">validate[length[0,100],custom[onlyNumberDot2]]</c:if>" type="text" <c:if test="${ makeTmp.jlsbz eq JLSBZ}">disabled="disabled"</c:if> value="${makeTmpItem.jlsbz }" />
							    	</td>
							    	<td class="padding0">
							    		<input class="num_input <c:if test="${!(makeTmp.jlqcl eq JLQCL)}">validate[length[0,6],custom[onlyNumberDot2]]</c:if>" type="text" <c:if test="${ makeTmp.jlqcl eq JLQCL}">disabled="disabled"</c:if> value="${makeTmpItem.jlqcl }" />
							    	</td>       
							    </c:if>
...
<!-- 					            <td class="padding0" style="color: #006699;width: 5%">确认</td> -->
					        ...
					        <c:forEach items="${zyp.makePersonKhList }" var="khPerson">
								<tr class="khPersonTr" personKhId="${khPerson.personKhId }">
								    ...
								    <td class="padding0" title="${khPerson.khDesc }"><input class="num_input validate[length[0,300]]" type="text" value="${khPerson.khDesc }" /></td>       
								    <td class="padding0"><input class="num_input validate[length[0,6],custom[onlyNumberDot2]]" type="text" value="${khPerson.khScore }" /></td>
<!-- 								    <td class="padding0"> -->
			...
			if(saveType == 2){	//提交必填
				$(".text_input:enabled, .num_input:enabled").each(function(){
					if($(this).attr("class").indexOf("validate[required,")==-1){
						$(this).attr("class",$(this).attr("class").replace("validate[","validate[required,"));
					}
				});
				$("#cxMaketbForm").validationEngine();
			}else{	//保存非必填
				$(".text_input:enabled, .num_input:enabled").each(function(){
					$(this).attr("class",$(this).attr("class").replace("validate[required,","validate["));
