...
<body>
	<div id="layout1">
		<div id="centerCon" position="center" style="">
			...
			<form action="${path}/zypQuery/getZypList" id="queryForm" method="post">
	...
	function initGrid(){
		...
		grid = $("#dataBasic").quiGrid({
			columns:[	
					 ...
					 { display: '负责人', name: 'fzr_name', align: 'center', width: "15%", showTitle:true},
					 { display: '拟定人', name: 'tbry_name', align: 'center', width: '15%', showTitle:true},
					 { display: '厂所', name: 'belong_wsc_name', align: 'center', width: '20%', showTitle:true},
					 { display: '作业票状态', name: 'cxzyp_status_name', align: 'center',width: 80, showTitle:true},
					 { display: '说明', name: 'zyp_desc', align: 'center',width: '30%', showTitle:true},
					 { display: '操作', isAllowHide: false, align: 'center', width:80,
					    	render: function (rowdata, rowindex, value, column){
								 var html ='<div class="padding_top4 padding_left5">'
	                               	 + '<span class="img_list hand" title="查看" onclick="detailAndProcess(\'' + rowdata.cx_make_id + '\')"></span>'
	                               	 + '</div>';
	                               	 return html;
			                 }
					    }
		    		],
		    url: '${path}/zypQuery/getZypList',rownumbers:true,checkbox:false,selectRowButtonOnly:false,params:params,
		    height: '100%', width:"100%", pageSize:20, percentWidthMode:true, sortName:'zyp_date', sortOrder:'desc',	 
		    toolbar:{
		           	 items:[
						   ]
		   }
	...
	//查看
	function detailAndProcess(cxMakeId){
		var param = {};
		param.zypDate = jQuery("#zypDate").val();
		param.zypCode = jQuery("#zypCode").val();
		var backURL = encodeURIComponent("/zypQuery/init");
		window.location = "${path}/zypQuery/detailAndProcess?cxMakeId="+cxMakeId+"&backURL="+backURL+"&queryParam="+JSON.stringify(param);
	}
