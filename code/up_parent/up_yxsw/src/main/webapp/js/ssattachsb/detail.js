	...
	var SBSSRelation = window.SBSSRelation={
		...
		initGrid : function(){
			grid = $("#sbssRelationGrid").quiGrid({
			    ...
			    toolbar:{
			    	items:[
			    		{text: '新增', click: SBSSRelation.add, iconClass: 'icon_add',visible: btnPermissions.add_},
			    		{ line : true },
			    		{text: '删除', click: SBSSRelation.del, iconClass: 'icon_delete',visible: btnPermissions.remove_}
			      ]}
			});
		},
		...
		add : function(){
			if(onlyQuery){
				Dialog.alert("非厂站用户不能进行设施设备关联操作");
				return;
			}
			var diag = new Dialog();
			diag.Title = "设施设备关联新增";
			diag.URL = path+"/ssattachsb/toEqInfoList";
			diag.Height = 620;
			diag.Width = 1200;
			...
			diag.OKEvent=function(){
				...
				$.post(path+"/ssattachsb/doAdd",{"relations":JSON.stringify(relations)},function(result){
					...
					if(result.flag){
						grid.loadData();
						// 只要新增成功就要刷新设施树
						parent.updateNodeColor(rows.length);
					}
				});
				diag.close();
			}
		...
		del : function(){
			if(onlyQuery){
				Dialog.alert("非厂站用户不能进行设施设备关联删除操作");
				return;
			}
			var allRowLength = grid.getData().length;
			var rows = grid.getSelectedRows();
			if(rows.length==0){
				Dialog.alert("请选择要删除的关联关系");
				return;
			}
			Dialog.confirm("是否从该设施下删除选定设备",function(){
				var IDS = new Array();
				for ( var i in rows) {
				...
				$.post(path+"/ssattachsb/doDel",{"ids":IDS.join(",")},function(result){
					...
					if(result.flag){
						grid.loadData();
						// 删除成后，判断剩余几条是否需要更新设施树
						if(allRowLength == rows.length){
							parent.updateNodeColor(0);
						}
					}
				});
			});
