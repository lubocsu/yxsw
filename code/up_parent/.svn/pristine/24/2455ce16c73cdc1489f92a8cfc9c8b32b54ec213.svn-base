...
/*
		...
		List<Map<String, Object>> typeList = this.getTypeList(csOrgId, sbTypeId, sbStatus, idNew);
	...
	protected List<Map<String,Object>> getTypeList(String csOrgId, String sbTypeId,String sbStatus,String idNew){
		 Map<String,Object> par=new HashMap<String,Object>();
			StringBuilder sql=new StringBuilder();
	...
	public List<String> getRenderList(String csOrgId, String sbTypeId,
		...
		if(null!=typeList&&typeList.size()>0){
			...
			for (Map<String, Object> mapA : typeList) {
				StringBuffer sql2 = new StringBuffer();
		        String id=mapA.get("sb_type_id").toString();
			...
			// Long count = sbBaseinfoDAO.queryCountBySql(sql2.toString(), params);
			List<Map<String, Object>> queryListBySql = sbBaseinfoDAO.queryListBySql(sql2.toString(), params);
			 //表示有数据 还要判断是是子节点
			 if(null!=queryListBySql&&queryListBySql.size()>0){
				  Map<String, Object> parA=new HashMap<String, Object>();
				 
				 StringBuilder sqlA=new StringBuilder();
				 sqlA.append(" select t.sb_type_id  from  BIZ_T_SB_TYPES t where t.parent_type_id=:idA ");
				 parA.put("idA", id);
				 List<Map<String, Object>> list = sbBaseinfoDAO.queryListBySql(sqlA.toString(), parA);
				 if(null!=list&&list.size()>0){
