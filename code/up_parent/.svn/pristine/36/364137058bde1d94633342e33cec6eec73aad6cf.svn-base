	...
	if (nodes && nodes.length > 0) {
		top.Dialog.confirm("确认允许访问  " + nodes[0].name + " 吗？",function(){
	        ...
	        $.post(path + "/permissionData/savePermissionData",
	        	{"type":2, "orgId":nodes[0].id, "relateId":relateId, "status":"1"},
	        	function(result){
	        		if(result.success){
	        			// 刷新页面
...
function delPermission(){
	...
	if (nodes && nodes.length > 0) {
		top.Dialog.confirm("确认禁止访问  " + nodes[0].name + " 吗？",function(){
            ...
            //此处进行ajax后台数据处理
            $.post(path + "/permissionData/savePermissionData", 
            	{"type":2, "orgId":nodes[0].id, "relateId":relateId, "status":"0"}, 
            	function(result){
            		if(result.success){
	        			// 刷新页面
		        		window.location.reload(true);
	...
	if(checkedNodes.length == 0 ){
		...
		top.Dialog.confirm(msg, function(){
	        ...
	        $.post(path + "/permissionData/savePermissionDataList", 
	        	{"type":2, "orgIds":ids, "relateId":relateId, "status":"1"}, 
	        	function(result){
	        		if(result.success){
	        			// 刷新页面
		        		window.location.reload(true);
...
function delSelectPermissions(){
	...
	if(checkedNodes.length == 0 ){
		...
		top.Dialog.confirm(msg, function(){
			...
			var relateId = $("#relateId").val();
	        $.post(path + "/permissionData/savePermissionDataList", 
	        	{"type":2, "orgIds":ids, "relateId":relateId, "status":"0"}, 
	        	function(result){
	        		if(result.success){
	        			// 刷新页面
		        		window.location.reload(true);
	...
	if(checkedNodes.length == 0 ){
		...
		top.Dialog.confirm(msg, function(){
	        ...
	        $.post(path + "/permissionData/delPermissionDataList", 
	        	{"type":2, "orgIds":ids, "relateId":relateId}, 
	        	function(result){
	        		if(result.success){
	        			// 刷新页面
		        		window.location.reload(true);
