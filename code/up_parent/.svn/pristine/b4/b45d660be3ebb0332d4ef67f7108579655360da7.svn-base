...
<script type="text/javascript">
	...
	function initGrid(){
		grid = $("#dataBasicp").quiGrid({
			columns:[	
						{ display: '类型', name: 'detail_type', align: 'left', width: "20%",showTitle:true},
						{ display: '名称', name: 'name', align: 'left', width: "30%",showTitle:true},
						{ display: '是否必扫标签', name: 'must_scan', align: 'center', width: "10%"},
						{ display: '确认方式', name: 'ewm_confirmed_type', align: 'center', width: "10%"},
						{ display: '是否完成', name: 'have_complete', align: 'center', width: "5%"},
						{ display: '操作时间', name: 'opt_time', align: 'center', width: "20%",type:"time"},
		    ...
		    url: '${path}/patrollingRecords/getSbssData?taskItemId=${taskItemId }',rownumbers:true,checkbox:true,selectRowButtonOnly:false,
	...
	//查看
	function onView(ttask_item_sbss_id){
		var url="${path}/patrollingRecords/toViewCheckInfo?ttaskItemSbssId="+ttask_item_sbss_id;
		window.open(url);
	}
