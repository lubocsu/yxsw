...
<script type="text/javascript">
	...
	function initGrid(){
		grid = $("#dataBasic").quiGrid({
			columns:[	
					    { display: '设备名称', name: 'sb_name', align: 'left', width: "30%",showTitle:true},
					    ...
					    { display: '安装地址', name: 'set_address', align: 'center', width: "40%",showTitle:true},
		    		],
		    url: '${path}/spotRelate/getNoRelatedData',rownumbers:true,checkbox:true,selectRowButtonOnly:false,
...
// 		           	        {line  : true }
	...
	function addEq(refresh) {
	    ...
	    for(var i = 0 ; i < rowsLength ; i++ ){
	    	ids += ( rows[i].sb_id + "," );
	    	names += ( rows[i].sb_name + "," );
	    }
	    $.ajax({
            cache: true,
            type: "POST",
            url:'${path}/spotRelate/doRelateEq',
