...
<body>
	<div id="layout1">
		<div id="centerCon" position="center" style="">
			...
			<form action="${path}/zypCheck/getZypCheckList" id="queryForm" method="post">
					 ...
					 { display: '操作', isAllowHide: false, align: 'center', width:80,
					    	render: function (rowdata, rowindex, value, column){
								 var html ='<div class="padding_top4 padding_left5">'
	                               	 + '<span class="img_list hand" title="查看并审核" onclick="detailAndCheck(\'' + rowdata.cx_make_id + '\')"></span>'
// 	                                 + '<span class="img_edit hand" title="审核" onclick=zypCheck(\''+ rowdata.cx_make_id +'\')></span>'
	                               	 + '</div>';
	                               	 return html;
			                 }
					    }
		    		],
		    url: '${path}/zypCheck/getZypCheckList',rownumbers:true,checkbox:false,selectRowButtonOnly:false,params:params,
	...
	function detailAndCheck(cxMakeId){
		...
		var backURL = encodeURIComponent("/zypCheck/init");
		window.location = "${path}/zypCheck/detailAndCheck?cxMakeId="+cxMakeId+"&backURL="+backURL+"&queryParam="+JSON.stringify(param);
	}
	
	//审核
// 	function zypCheck(cxMakeId){
// 		if(csOrgType != "3"){
// 			Dialog.alert("非厂站用户不能审核！");
// 			return ;
// 		}
// 		param.zypDate = jQuery("#zypDate").val();
// 		param.zypCode = jQuery("#zypCode").val();
// 		var backURL = encodeURIComponent("/zypCheck/init");
// 		window.location = "${path}/zypCheck/zypCheck?cxMakeId="+cxMakeId+"&backURL="+backURL+"&queryParam="+JSON.stringify(param);
// 	}
