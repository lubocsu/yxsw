	                 ...
	                 <form action="#" id="checkItem_content">
	                 	<table class="detail-list">
		                 	<tr>
				    			...
				    			<td>
				    				<input type="text" name="checkItemName" id="checkItemName" class="validate[required,length[0,42]]" value="" style="width: 253px;" />
				    			</td>
		                 		<th><span class="red">*</span>检查项编号：</th>
				    			<td>
				    				<input type="text" name="checkItemCode" id="checkItemCode" class="validate[required,custom[notChinese],length[0,10],ajax[${path }/checkitem/validateCheckItemCode|* 检查项编码已存在!]]"  value="" style="width: 253px;"/>
				    			</td>
		                 	</tr>
		                    <tr>
		                        <th><span class="red">*</span>检查项类型：</th>
								<td>
									<select labelField="value" valueField="key" selWidth="253" name="checkItemType" id="checkItemType" class="validate[required]"  prompt="请选择" url="${path}/platform/getSupportData" params='{"parentNodeId":"${CHECKITEM_TYPE }"}' ></select>
								</td>
		                        <th><span class="red">*</span>输入类型：</th>
								<td>
									<select labelField="value" valueField="key"  selWidth="253" name="inputType" id="inputType" class="validate[required]"  prompt="请选择" url="${path}/platform/getSupportData" params='{"parentNodeId":"${CHECKITEM_INPUTTYPE }"}' ></select>
								</td>
		                    </tr>
		                    ...
		                    <tr>
		                        <th>检查项说明：</th>
				    			<td colspan="3">
				    				<textarea name="checkItemDesc" id="checkItemDesc"  maxNum="141" style="width:76%;" ></textarea>
				    			</td>
		                    </tr>
		                 </table>
	                 </form>
	            ...
	            <div class="task-audit" id='checkItemDetal_div' style="display:none;">
					...
					<form action="#" id="checkItemDetail_content">
						<table class="detail-list" id="check_item_detail_table">
				            <tr style="background-color: #c6e5fb;" id="check_item_detail_head">
	...
	<div class="list-btm">
	  	<div class="list-btm-div">
	        <div class="div-btn-sbt">
	        	 <button class="btm-button" type="button"  onclick="doAddCheckItemDetail(this);"><span class="ok-icon">保存</span></button>
	...
	<script type="text/javascript">
		...
		jQuery(function(){
			...
			jQuery("#checkItemKxx").on("change",function(){
				...
				if(id.length==0){
					sel_sort = 1;
					var check_item_detail_head = jQuery("#check_item_detail_head"); 
					jQuery("#check_item_detail_table").empty();
					jQuery("#check_item_detail_table").append(check_item_detail_head);
					return;
				}
