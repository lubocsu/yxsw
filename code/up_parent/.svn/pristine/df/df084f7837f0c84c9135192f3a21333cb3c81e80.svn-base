	...
	function doAddTechnicsScope(){
		...
		if(valid){
			$.ajax({
	            cache: true,
	            type: "POST",
	            url:'${path}/technicsScope/doAddTechnicsScope',
	            data:$('#myFormId').serialize(),// 你的formid
	            async: false,
	            error: function(request) {
	            	parent.Dialog.alert("保存失败");
	            },
	            success: function(responseText, statusText, xhr, $form){
	            	parent.Dialog.alert(responseText.message, function(){
		            		var technicsId = responseText.technicsId;
		                	//刷新数据
		                	parent.refresh(technicsId);
		                    //关闭窗口
		                    parent.Dialog.close();
		                    parent.jQuery("#detail-iframe").attr("src","${path}/technicsScope/showDetail?technicsId="+ technicsId );
	                });
	            }
	        });
		}
	                 ...
	                 <div class="to_hide_over_content" class="text">
		                 ...
		                 <table class="detail-list" >
		                 	<tr>
		                 		<th>厂站：</th>
		                 		<td>
		                 			<input class="textbox" name="csOrgName" id="csOrgName" value="${csOrgName }" type="text" readonly="readonly" style="width:180px;"/>
									<input type="hidden" name="csOrgId" id="csOrgId" value="${csOrgId }" />
		                 		</td>
		                 		<th><span><em style="color:red;">*</em></span>工艺段名称：</th>
		                 		<td><input type="text" id="technicsName" name="technicsName" class="validate[required,length[0,40]]" value=""  style="width:180px;"/></td>
		                 	</tr>
		                 	<tr>
								<th>工艺段说明：</th>
								<td colspan="3">
									<textarea maxNum="300" name="technicsDesc" class="validate[length[0,300]]"  style="width:73%;height:110px;"/></textarea>
								</td>
							</tr>
		                 </table>
	            	</div>
		...
		<div class="list-btm">
	    	<div class="list-btm-div">
	            <div class="div-btn-sbt">
	            	<button class="btm-button" type="button"  onclick="doAddTechnicsScope();"><span class="ok-icon">保存</span></button>
	            </div>
	        </div>
	    </div>
     </div>
