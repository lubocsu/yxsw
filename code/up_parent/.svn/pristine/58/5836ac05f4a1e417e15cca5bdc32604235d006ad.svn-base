	...
	<div class="details-max">
		<div class="list-content">
	    	 <div class="details-container">
                <form action="${path}/freq/saveEditFreq" id="form">
                	<input type="hidden" name="freqSegmentId" id="freqSegmentId" value="${freq.freqSegmentId }"/>
                	<input type="hidden" name="updateTimestemp" id="updateTimestemp" value="${freq.updateTimestemp }"/>
		                ...
		                <div class="to_hide_over_content" class="text">
	                 	...
	                 	<table class="detail-list">
		                 	<tr>
				    			...
				    			<td>
				    				<input type="text" name="freqSegmentName" id="freqSegmentName" class="validate[required,length[0,42]]" value="${freq.freqSegmentName }" style="width: 253px;" />
				    			</td>
		                 		<th><span class="red">*</span>所属班次：</th>
				    			<td>
									<select labelField="detail_name" valueField="detail_id" selWidth="253" name="detailId" id="detailId" class="validate[required]" selectedvalue="${freq.detailId }"  prompt="请选择" url="${path}/workPeriod/getWpDetail"></select>
									<input type="hidden" name="detailName" id="detailName" value="${freq.detailName }"/>
								</td>
		                 	</tr>
		                    <tr>
		                        <th><span class="red">*</span>开始时间：</th>
		                        <td>
		                        	<!-- <input type="text" class="date validate[required]" name="startTime" id="startTime"  value="" disabled="disabled" style="width:253px" readonly="readonly" onFocus="WdatePicker({dateFmt: 'HH:mm',isShowClear:true})" /> -->
		                        	<!-- <input type="text" class="date validate[required]" name="startTime" id="startTime"  value="" disabled="disabled" style="width:253px" readonly="readonly" onFocus="WdatePicker({skin:themeColor,dateFmt:'HH:mm',minTime:'09:00:00',maxTime:'17:30:00'})" /> -->
		                        	<input type="text" class="date validate[required]" name="startTime" id="startTime"  value="${freq.startTime }" style="width:253px" readonly="readonly" onFocus="WdatePicker({skin:themeColor,dateFmt: 'HH:mm',isShowClear:true,onpicked:vlpick})""/>
		                        	<br><span id="timeScope_start"></span>
		                        </td>
		                        <th><span class="red">*</span>结束时间：</th>
								<td>
									<!-- <input type="text" class="date validate[required]" name="endTime" id="endTime"  value="" disabled="disabled" style="width:253px" readonly="readonly" onFocus="WdatePicker({skin:themeColor,dateFmt: 'HH:mm',isShowClear:true, minTime:'#F{dateStr2Str(wpdNode.start_time,\'$1:$2\', /(\d{2})(\d{2})/)}'})" /> -->
									<!-- <input type="text" class="date validate[required]" name="endTime" id="endTime"  value="" disabled="disabled" style="width:253px" readonly="readonly" onFocus="WdatePicker({skin:themeColor,dateFmt: 'HH:mm',isShowClear:true, minTime:'#F{getMinTime()}', maxTime:'#F{getMaxTime()}'})" /> -->
									<input type="text" class="date validate[required]" name="endTime" id="endTime"  value="${freq.endTime }" style="width:253px" readonly="readonly" onFocus="WdatePicker({skin:themeColor,dateFmt:'HH:mm',isShowClear:true,onpicked:vlpick})"/>
									<br><span id="timeScope_end"></span>
								</td>
		                    </tr>
		                    <!-- <tr>
		                        <th>任务说明：</th>
		            ...
		            <div class="task-audit" id="freq_detail">
						...
						<div class="detail-list">
							<div  class="basicTab" style="background-color: rgb(241, 244, 255);"  iframeMode="true" 
								data='{"list":[{"name":"巡检设备","url":"${path }/freq/toFreqEpList?freqId=${freq.freqSegmentId }"},
						      			{"name":"巡检设施","url":"${path }/freq/toFreqSsList?freqId=${freq.freqSegmentId }"}]}'>
