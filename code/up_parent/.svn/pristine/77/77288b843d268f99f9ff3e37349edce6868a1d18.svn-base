...
<body>
	<div id="layout1">
		<div id="centerCon" position="center" style="">
			...
			<form action="${path}/zypDiscontinue/getZypDiscontinueList" id="queryForm" method="post">
...
<%-- 							<input type="text" name="zypDate" id="zypDate" style="width:140px" value="${zyp.zypDate }" /> --%>
					 ...
					 { display: '操作', isAllowHide: false, align: 'center', width:80,
					    	render: function (rowdata, rowindex, value, column){
								 var html ='<div class="padding_top4 padding_left5">'
	                               	 + '<span class="img_list hand" title="查看并中止" onclick="detailAndDiscontinue(\'' + rowdata.cx_make_id + '\')"></span>'
	                               	 + '</div>';
	                               	 return html;
			                 }
					    }
		    		],
		    url: '${path}/zypDiscontinue/getZypDiscontinueList',rownumbers:true,checkbox:false,selectRowButtonOnly:false,params:params,
	...
	//查看并中止
	function detailAndDiscontinue(cxMakeId){
		var param = {};
		param.zypDate = jQuery("#zypDate").val();
		param.zypCode = jQuery("#zypCode").val();
		var backURL = encodeURIComponent("/zypDiscontinue/init");
		window.location = "${path}/zypDiscontinue/detailAndDiscontinue?cxMakeId="+cxMakeId+"&backURL="+backURL+"&queryParam="+JSON.stringify(param);
	}
