...
<body>
	...
	<script>
	    ...
	    value: [today]
			...
			$.ajax({
	            type: "POST",
	            url: "${path}/mobile/xjItemScan/getXjItemRecordList",
	            data: {
	            		"tokenId":"${tokenId}",
	            		"xjItemId":"${xjItemId}",
	            		"pager.pageNo":pageNo,
	            ...
	            success: function (appresult) {
	                ...
	                if(flag){
	                	var xjItemRecordList = appresult.data.xjItemRecordList;
	                	//生成工单数据
	                	...
	                	for(var i in xjItemRecordList){
	                		var xjItemList = '<li onclick="toXjItemDetail(\''+ xjItemRecordList[i].task_item_id +'\');">';
	                		xjItemList += '<div class="company-top"><h3>'+(xjItemRecordList[i].xjd_item_name!=null?xjItemRecordList[i].xjd_item_name:"--")+'</h3></div>';
	                		xjItemList += '<div class="company-cont"><table>';
	                		xjItemList += '<tr><th>RFID编号：</th><td  style="width:50%;">'+(xjItemRecordList[i].rfid_code != null?xjItemRecordList[i].rfid_code:"--")+'</td></tr>';
	                		xjItemList += '<tr><th>是否已完成：</th><td>'+(xjItemRecordList[i].have_complete != null?xjItemRecordList[i].have_complete:"--")+'</td></tr>';
	                		xjItemList += '<tr><th>操作时间：</th><td>'+(xjItemRecordList[i].opt_time != null?xjItemRecordList[i].opt_time:"--")+'</td></tr>';
	                		xjItemList += '</table></div></li>';
	                		listsul.append(xjItemList);
	                	}
	                	
		                if(udflag=="up"){
	  	  	  				func(xjItemRecordList.length,pageSize,true);
	  	  	  			}else if(udflag=="down"){
	  	  					func(xjItemRecordList.length,pageSize,true);
	  	  	  	  		}
	                }else{
		...
		function toXjItemDetail(taskItemId){
			var url = "${path}/mobile/xjItemScan/toTaskItemDetail?taskItemId=" + taskItemId + "&tokenId=${tokenId}";
			location.href = url;
		}
