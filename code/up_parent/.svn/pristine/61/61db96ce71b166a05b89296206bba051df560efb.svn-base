...
public class OrgServiceImpl extends BaseServiceImpl implements OrgService, ExportExcelDataSource {
	...
	public SysOrgEntity save2(SysOrgEntity sysOrg) {
		if(!StringUtils.equals("0",sysOrg.getParentOrgId())){
			SysOrgEntity pOrg = findOneById(sysOrg.getParentOrgId());
			sysOrg.setOrgCode(pOrg.getOrgCode()+"_"+sysOrg.getOrgCode());
		}
		return save(sysOrg);
	}
	...
	public SysOrgEntity update2(SysOrgEntity sysOrg) {
		// 取得原有机构编码
		String oldParentOrgCode = findOneById(sysOrg.getOrgId()).getOrgCode();
		// 判断更新本机构编码时是否需要加上新父级机构编码
		if(!StringUtils.equals("0",sysOrg.getParentOrgId())){
			SysOrgEntity pOrg = findOneById(sysOrg.getParentOrgId());
			sysOrg.setOrgCode(pOrg.getOrgCode()+"_"+sysOrg.getOrgCode());
		}
		SysOrgEntity ss = update(sysOrg);
		// 更新子机构编码前缀
		if(!StringUtils.equals(oldParentOrgCode, ss.getOrgCode())){
			
			StringBuilder sql = new StringBuilder();
			sql.append("SELECT * FROM  SYS_ORG O WHERE O.ORGID <> '"+ss.getOrgId()+"' ");
			sql.append("  START WITH O.ORGID = '"+ss.getOrgId()+"' CONNECT BY PRIOR O.ORGID=O.PARENTORGID ");
			List<SysOrgEntity> cOrgList = orgDao.queryListBySql(sql.toString(), new HashMap<String, Object>(), SysOrgEntity.class);
			for (SysOrgEntity s : cOrgList) {
				s.setOrgCode(s.getOrgCode().replace(oldParentOrgCode+"_", ss.getOrgCode()+"_"));
				update(s);
			}
		}
		userService.updateUserByOrg(ss.getOrgId(), ss.getEnabled());
		return ss;
	}
}
