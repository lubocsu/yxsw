...
<script type="text/javascript">
	...
	function initGrid(){
		grid = $("#dataBasicp").quiGrid({
			columns:[	
					    { display: '设备名称', name: 'sb_name', align: 'left', width: "30%"},
					    ...
					    { display: '性能参数', name: 'xncs', align: 'center', width: "20%"},
					    { display: '安装地址', name: 'set_address', align: 'center', width: "10%"},
		    		],
		    url: '${path}/spotRelate/getRelatedEqData?spotId=${spotId }',rownumbers:true,checkbox:true,selectRowButtonOnly:false,
...
// 			var diag = new Dialog();
			diag.Title = "新增设备";
			diag.URL = "${path }/spotRelate/toAddEq?spotId=${spotId }";
			diag.Width = 900;
			diag.Height = 500;
			//设置保存按钮文字（默认是确定）
			...
			diag.OKEvent = function(){
				//调用子页面submitForm 方法
				diag.innerFrame.contentWindow.addEq(refresh);
			};
			diag.show();
		}
	}
