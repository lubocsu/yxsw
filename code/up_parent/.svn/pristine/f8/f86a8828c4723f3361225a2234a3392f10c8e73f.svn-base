...
<body>
	...
	<div id="layout1" style="width:100%">
	  <div id="centerCon" position="center" style="width:100%">
	  	<div class="box2" panelTitle="人员巡检情况统计" showStatus="false">
					...
					<table align="center">
						...
						<tr>
							...
							<td colspan="3">
								...
								<input type="radio" name="dateType" id="one-work" value="7"/><label for="one-work">本周</label>
								<input type="radio" name="dateType" id="one-m" value="6"/><label for="one-m">本月</label>
								<input type="radio" name="dateType" id="one-y" value="8"/><label for="one-y">本年</label>
							...
							<table class="tableStyle" mode="list">
								...
								<%--  <input  type="hidden" id="csId" value="${map.belong_wsc_id}"> </input> --%>
									<tr>
										...
										<td  align="center"  mid="${map.belong_wsc_name }" class="business"><a  onclick="getPersonDetail('${map.belong_wsc_id}')"   href="javascript:void(0);">${map.belong_wsc_name}</a></td>
...
<script type="text/javascript">
    var dateType='${reportForm.dateType}';
...
/* 	$(".business a").live("click", function() {
   ...
   function getPersonDetail(id){
	   ...
	   location.href = '${path}/piss/getPerson?csId='+ id +"&startTime=${st}&endTime=${et}"
